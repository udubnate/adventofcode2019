...
def parse_movement(movement,myList):
    instruction = movement.split(',')
    for mv in instruction:
        move(mv[0],int(mv[1:]),myList)
...
def compare_wires(w1,w2):
    ...
    for item1 in w1:
        for item2 in w2:
            if item1 == item2:
                print("found a crossing at :" + str(item1))
                taxicab = math.fabs(item1[0]) + math.fabs(item1[1])
                dist = w1count + w2count
                #print(taxicab)
                if taxicab == 0:
                    continue
                elif taxicab < short:
                    short = taxicab
                print("Shortest Distance :" + str(short))
                ...
                print("Shortest Total Path Distance : " + str(shortdist))
            w2count += 1
        w1count += 1
        w2count = 0

parse_movement(wire1movement,wire1)
parse_movement(wire2movement,wire2)
print(wire1)
compare_wires(wire1,wire2)